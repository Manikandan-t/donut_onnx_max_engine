...
class OnnxPredictor:
    ...
    def prepare_input(self, img):
        ...
        if self.align_long_axis and (
                (input_size[0] > input_size[1] and img.width > img.height) or
...
model_path = "/home/manikandan.tm@zucisystems.com/workspace/onnx-donut/export-int8"
model_config = OnnxPredictor(model_path)
max_length = model_config.max_length

encoder_model = session.load(os.path.join(f"{model_path}", "encoder.onnx"))
decoder_model = session.load(os.path.join(f"{model_path}", "decoder.onnx"))
decoder_past_model = session.load(os.path.join(f"{model_path}", "decoder_with_past.onnx"))
tokenizer = XLMRobertaTokenizerFast.from_pretrained(model_path)


def extract_index(key):
    # Extracts the numeric index from the key
...
async def generate(questions: str, image: UploadFile = File(...)):
    try:
        ...
        encoder_input_ids = model_config.prepare_input(Image.fromarray(img_arr))
        ...
        for prompt in prompts:
            ...
            input_ids = tokenizer(prompt, add_special_tokens=False, return_tensors="np").input_ids.astype(dtype='int32')
            ...
            encoder_outputs_node = encoder_model.execute(**inputs)
            encode_element = next(iter(encoder_outputs_node))
            encoder_outputs = encoder_outputs_node[encode_element]
            ...
            while not stop:
                ...
                decoder_outputs_node = decoder_model.execute(**decoder_input)
                ...
                # else:
                #     print("Past keys is not none")
                #     decoder_past_input = {
                #         'input_ids': input_ids[:, -1:],
                #         **past_key_values
                #     }
                #     decoder_outputs_node = decoder_past_model.execute(**decoder_past_input)
                #     decoder_outputs_node_list = list(decoder_outputs_node.values())
                #     past_key_values = {'past_key_value_input_' + str(i): pkv for i, pkv in enumerate(decoder_outputs_node_list[1:])}
            ...
            seq = tokenizer.batch_decode(input_ids)[0]
            seq = seq.replace(tokenizer.eos_token, "").replace(tokenizer.pad_token, "")
            seq = re.sub(r"<.*?>", "", seq, count=1).strip()  # remove first task start token
            results.append(OnnxPredictor.token2json(seq, tokenizer))
